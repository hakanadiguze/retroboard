...
import { db, auth, uid, nowISO, roomRef, fbGet, fbSet, signInWithGoogle, signOutUser, onAuth, getAllTeams } from "./firebase.js";
...
async function exportPDF(room) {
  ...
  doc.text(`Generated: ${new Date().toLocaleString()}   |   Room: ${room.id}${room.teamName?" | Team: "+room.teamName:""}`,M,22);
  y=36;
  ...
  questions.forEach((q,qi)=>{
    ...
    qi%2===0?sf(T.offWhite):sf(T.white);
    ...
    parts.forEach((p,i)=>{
      sc(T.dark);
      doc.text(String(p.scores?.[q.id]??"-"),M+qColW+i*dataColW+dataColW/2,y+6,{align:"center"});
    });
    sf(c); doc.rect(avgX,y,dataColW,rowH,"F");
    sc(T.white); doc.setFont("helvetica","bold");
    doc.text(avg(q.id),avgX+dataColW/2,y+6,{align:"center"});
    y+=rowH;
  });
  y+=10;
...
function BoardColumn({ col, entries, onMove, onReorder, onAddAction, actions, selectedCardIds, onToggleCard }) {
  ...
  function handleDrop(e){
    ...
    if(!entryId) return;
    if(fromCol===col){if(fromIdx>=0&&overIdx!==null&&fromIdx!==overIdx)onReorder(col,fromIdx,overIdx);}
    else onMove(entryId,col,overIdx);
    setOverIdx(null);
  }
  ...
  return (
    <div style={{flex:1,minWidth:280,background:T.white,borderRadius:16,boxShadow:`0 4px 20px ${T.teal}15`,overflow:"hidden"}}>
      <div style={{background:c,padding:"12px 16px",display:"flex",alignItems:"center",gap:8}}>
        ...
        {hasSelection&&<span style={{background:"rgba(255,255,255,.3)",color:T.white,borderRadius:10,padding:"1px 8px",fontSize:11,fontWeight:700}}>{selectedHere.length} selected</span>}
      ...
      <div onDragOver={e=>handleDragOver(e,entries.length)} onDragLeave={()=>{setOver(false);setOverIdx(null);}} onDrop={handleDrop}
        ...
        {entries.map((entry,idx)=>{
          ...
          return(
            <div key={entry.id}>
              {over&&overIdx===idx&&<div style={{height:3,background:c,borderRadius:4,marginBottom:6,opacity:.8}}/>}
              <div draggable
                onDragStart={e=>{e.dataTransfer.setData("entryId",entry.id);e.dataTransfer.setData("fromCol",col);e.dataTransfer.setData("fromIdx",String(idx));}}
                onDragOver={e=>handleDragOver(e,idx)}
                onClick={e=>{e.stopPropagation();onToggleCard(entry.id);}}
                style={{background:isSelected?`${c}15`:T.offWhite,borderRadius:10,padding:"10px 12px",marginBottom:8,
                  border:isSelected?`2px solid ${c}`:`1.5px solid ${c}18`,cursor:"pointer",fontSize:14,
                  boxShadow:isSelected?`0 0 0 3px ${c}28,0 3px 10px rgba(0,0,0,.08)`:"0 2px 6px rgba(0,0,0,.04)",
                  transition:"all .15s",userSelect:"none"}}>
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:5}}>
                  <div style={{width:16,height:16,borderRadius:4,flexShrink:0,border:`2px solid ${isSelected?c:T.gray300}`,background:isSelected?c:"transparent",display:"flex",alignItems:"center",justifyContent:"center",transition:"all .15s"}}>
                    {isSelected&&<span style={{color:"#fff",fontSize:10,lineHeight:1}}>✓</span>}
                  </div>
                  <span style={{background:c,color:"#fff",borderRadius:10,padding:"1px 8px",fontSize:10,fontWeight:700}}>{entry.participantName}</span>
                </div>
        ...
        {over&&overIdx===entries.length&&<div style={{height:3,background:c,borderRadius:4,marginTop:2,opacity:.8}}/>}
        {entries.length===0&&!over&&<div style={{textAlign:"center",color:T.gray300,fontSize:13,paddingTop:32,paddingBottom:16}}>Drop items here</div>}
      </div>
      <div style={{borderTop:`2px solid ${c}30`,padding:12,background:`${c}06`}}>
        ...
        {actions.map(a=>{
          ...
          return(
            <div key={a.id} style={{background:isHighlighted?`${c}14`:T.white,borderRadius:8,padding:"7px 10px",marginBottom:6,fontSize:13,color:T.gray700,border:isHighlighted?`1.5px solid ${c}`:`1px solid ${c}15`,transition:"all .2s"}}>
              <span>• {a.text}</span>
              {linkedEntries.length>0&&(
                <div style={{marginTop:4,display:"flex",flexWrap:"wrap",gap:4}}>
                  {linkedEntries.map(le=>(
                    <span key={le.id} style={{fontSize:10,color:c,fontWeight:700,background:`${c}18`,borderRadius:6,padding:"1px 7px"}}>
        ...
        <div style={{marginTop:6}}>
          {hasSelection&&(
            <div style={{fontSize:11,color:c,fontWeight:700,marginBottom:5,display:"flex",alignItems:"center",gap:4,flexWrap:"wrap"}}>
              <span>🔗</span><span>Will link to:</span>
              {selectedHere.map(e=>(
                <span key={e.id} style={{background:`${c}18`,borderRadius:6,padding:"1px 7px",fontSize:10}}>{e.participantName}: {e.text.slice(0,20)}{e.text.length>20?"…":""}</span>
              ))}
            </div>
          )}
          <div style={{display:"flex",gap:6}}>
            <input value={actText} onChange={e=>setActText(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleAddAction()}
              placeholder={hasSelection?"Add linked action…":"Add action…"}
              style={{flex:1,padding:"7px 10px",borderRadius:8,fontSize:13,color:T.dark,outline:"none",border:`1.5px solid ${hasSelection?c+"90":c+"40"}`,background:hasSelection?`${c}08`:T.white}}/>
            <button onClick={handleAddAction} style={{padding:"7px 10px",borderRadius:8,border:"none",background:c,color:T.white,cursor:"pointer",fontWeight:700,fontSize:13}}>+</button>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap"}}>
        {questions.map((q,i)=>(
          <div key={q.id} style={{flex:"1 1 140px",background:T.white,borderRadius:14,padding:"14px 16px",boxShadow:`0 3px 14px ${colors[i%colors.length]}20`,borderTop:`4px solid ${colors[i%colors.length]}`}}>
        ...
        <table style={{width:"100%",borderCollapse:"collapse",tableLayout:"auto"}}>
          <thead>
            <tr style={{background:T.teal}}>
              <th style={{padding:"10px 16px",textAlign:"left",color:T.white,fontWeight:700,fontSize:13,whiteSpace:"nowrap",minWidth:180}}>Question</th>
              {parts.map(p=><th key={p.name} style={{padding:"10px 8px",textAlign:"center",color:T.white,fontWeight:700,fontSize:12,minWidth:60,whiteSpace:"nowrap"}}>{p.name}</th>)}
              <th style={{padding:"10px 8px",textAlign:"center",color:T.white,fontWeight:800,fontSize:12,minWidth:60,background:"rgba(0,0,0,.15)",whiteSpace:"nowrap"}}>Avg</th>
            </tr>
          </thead>
          <tbody>
            {questions.map((q,qi)=>{
              ...
              return(
                <tr key={q.id} style={{background:qi%2===0?T.offWhite:T.white}}>
                  <td style={{padding:"10px 16px",fontWeight:600,color:T.dark,fontSize:13,borderLeft:`4px solid ${c}`}}>{q.label}</td>
                  {parts.map(p=>(
                    <td key={p.name} style={{padding:"10px 8px",textAlign:"center"}}>
                      <span style={{background:c,color:T.white,borderRadius:8,padding:"3px 10px",fontWeight:800,fontSize:14,display:"inline-block"}}>{p.scores?.[q.id]??"-"}</span>
                    </td>
                  ))}
                  <td style={{padding:"10px 8px",textAlign:"center",background:`${c}15`}}>
                    <span style={{background:c,color:T.white,borderRadius:8,padding:"3px 10px",fontWeight:900,fontSize:14,display:"inline-block"}}>{avg(q.id)}</span>
                  </td>
                </tr>
              );
            })}
...
function SetupScreen({ hostName, onBack, onCreate, teams, isAdmin }) {
  ...
  function removeQuestion(idx){if(questions.length<=1)return;setQuestions(qs=>qs.filter((_,i)=>i!==idx));}
      ...
      <div style={{maxWidth:640,margin:"0 auto"}}>
        ...
        <div style={{background:T.white,borderRadius:20,padding:28,boxShadow:`0 6px 32px ${T.teal}15`}}>
            ...
            <div style={{marginBottom:20}}>
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                <button onClick={()=>setTeamId("")}
                  style={{padding:"7px 14px",borderRadius:10,border:`2px solid ${!teamId?T.teal:T.gray100}`,background:!teamId?T.tealBg:T.white,color:!teamId?T.tealDark:T.gray500,fontWeight:700,fontSize:13,cursor:"pointer"}}>
                  No Team
                </button>
                {teams.map(t=>(
                  <button key={t.id} onClick={()=>setTeamId(t.id)}
                    style={{padding:"7px 14px",borderRadius:10,border:`2px solid ${teamId===t.id?T.teal:T.gray100}`,background:teamId===t.id?T.tealBg:T.white,color:teamId===t.id?T.tealDark:T.gray500,fontWeight:700,fontSize:13,cursor:"pointer"}}>
                    {t.name}
                  </button>
                ))}
              </div>
          ...
          <div style={{marginBottom:20}}>
            ...
            {questions.map((q,idx)=>(
              <div key={q.id} style={{background:T.gray50,borderRadius:12,padding:14,marginBottom:10,border:`1.5px solid ${T.gray100}`}}>
                <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:10}}>
                  ...
                  <input value={q.label} onChange={e=>updateQ(idx,"label",e.target.value)} style={{...inp,fontWeight:700,fontSize:14,flex:1}}/>
                  {questions.length>1&&<button onClick={()=>removeQuestion(idx)} style={{background:"none",border:"none",color:T.gray300,cursor:"pointer",fontSize:20,lineHeight:1,padding:"0 4px"}}>×</button>}
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr auto",gap:8,alignItems:"center"}}>
                  <div><div style={{fontSize:11,color:T.gray500,marginBottom:3}}>Low label</div><input value={q.low} onChange={e=>updateQ(idx,"low",e.target.value)} style={inp}/></div>
                  <div><div style={{fontSize:11,color:T.gray500,marginBottom:3}}>High label</div><input value={q.high} onChange={e=>updateQ(idx,"high",e.target.value)} style={inp}/></div>
                  <div>
                    <div style={{fontSize:11,color:T.gray500,marginBottom:3}}>Scale</div>
                    <select value={q.scale||5} onChange={e=>updateQ(idx,"scale",parseInt(e.target.value))} style={{...inp,width:70}}>
            ...
            {questions.length<5&&(
              <button onClick={addQuestion} style={{width:"100%",padding:"10px",borderRadius:10,border:`2px dashed ${T.gray100}`,background:"none",color:T.gray500,fontWeight:700,fontSize:13,cursor:"pointer"}}>
                + Add Question (max 5)
              </button>
            )}
          </div>
  ...
  return (
    <div style={{minHeight:"100vh",background:"#E8F8F5",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'Segoe UI',system-ui,sans-serif",padding:"20px 16px"}}>
      <div style={{width:"100%",maxWidth:540,background:T.white,borderRadius:24,boxShadow:"0 8px 48px rgba(13,158,158,.13)",padding:"36px 32px",border:`1.5px solid ${T.tealLight}40`}}>
        ...
        {/* Admin login */}
        <div style={{borderTop:`1.5px solid ${T.gray100}`,paddingTop:14,marginTop:4,textAlign:"center"}}>
          {adminUser ? (
...
function JoinScreen({ onJoin, roomId }) {
  ...
  async function handleClick(){setLoading(true);await onJoin(name,setError);setLoading(false);}
  return (
    <div style={{minHeight:"100vh",background:"#E8F8F5",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'Segoe UI',system-ui,sans-serif",padding:"20px"}}>
      <div style={{width:"100%",maxWidth:460,background:"#fff",borderRadius:24,boxShadow:"0 8px 48px rgba(13,158,158,.12)",padding:"36px 32px"}}>
        <div style={{textAlign:"center",marginBottom:26}}>
          ...
          <h1 style={{fontSize:24,fontWeight:900,color:"#1a2e2e",margin:"0 0 4px"}}>{isRevealed?"View Results":"You're invited!"}</h1>
          <p style={{color:"#7a9a9a",margin:0,fontSize:14}}>{isRevealed?"This retrospective has ended. Enter your name to view the results.":"Enter your name to join this retrospective."}</p>
        </div>
        {isRevealed&&(
        ...
        <div style={{position:"relative"}}>
          <input value={name} onChange={e=>{setName(e.target.value.slice(0,MAX));setError("");}}
            onKeyDown={e=>e.key==="Enter"&&handleClick()} placeholder="Your name" autoFocus maxLength={MAX}
...
export default function App() {
  ...
  async function handleAdminLogin(){
    try{ await signInWithGoogle(); window.location.hash="admin"; }
  ...
  async function revealResults(){
    ...
    const boardEntries=[];
    Object.entries(r.participants||{}).forEach(([,p])=>{
      Object.entries(p.entries||{}).forEach(([col,list])=>{
        (list||[]).forEach((e,idx)=>boardEntries.push({id:uid(),participantId:"",participantName:p.name,text:e.text,column:col,order:idx}));
      });
    });
  ...
  async function moveEntry(entryId,newCol,atIdx){
    const r=await fbGet(roomId);
    let all=[...(r.boardEntries||[])];
    const entry=all.find(e=>e.id===entryId);if(!entry)return;
    all=all.filter(e=>e.id!==entryId);
    const col=all.filter(e=>e.column===newCol);
    const others=all.filter(e=>e.column!==newCol);
    const at=atIdx!=null?Math.min(atIdx,col.length):col.length;
    col.splice(at,0,{...entry,column:newCol});
    await update(roomRef(roomId),{boardEntries:[...others,...col]});
  }

  async function reorderEntry(col,fromIdx,toIdx){
    const r=await fbGet(roomId);
    const all=[...(r.boardEntries||[])];
    const colE=all.filter(e=>e.column===col);
    const others=all.filter(e=>e.column!==col);
    const [moved]=colE.splice(fromIdx,1);colE.splice(toIdx,0,moved);
    await update(roomRef(roomId),{boardEntries:[...others,...colE]});
  }
  ...
  function copyLink(){ const url=`${window.location.origin}${window.location.pathname}#retro-${roomId}`;navigator.clipboard.writeText(url).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2000);}); }
      ...
      <div style={base}>
        <div style={{maxWidth:820,margin:"0 auto",padding:"20px 16px 60px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:14,marginBottom:20}}>
            {COLUMNS.map(col=>{
              const c=COL_COLORS[col];
              const hints={Stop:{icon:"🛑",desc:"What is slowing us down? What should we stop doing?"},Start:{icon:"🚀",desc:"What should we try that we're not doing yet?"},Continue:{icon:"✅",desc:"What is working well and should continue?"}}[col];
              return(
  ...
  if(view==="board"&&room){
    ...
    function toggleCard(id){setSelectedCardIds(prev=>{const next=new Set(prev);next.has(id)?next.delete(id):next.add(id);return next;});}
    return(
      <div style={base}>
        <div style={{background:`linear-gradient(135deg,${T.tealDark},${T.teal})`,padding:"14px 24px",display:"flex",alignItems:"center",gap:14,boxShadow:`0 4px 20px ${T.teal}40`}}>
          ...
          <div style={{marginLeft:"auto",display:"flex",gap:8,alignItems:"center"}}>
            {selCount>0&&(
              <>
                <div style={{background:"rgba(255,255,255,.2)",borderRadius:10,padding:"6px 12px",fontSize:12,color:T.white,fontWeight:600}}>☑️ {selCount} selected</div>
                <button onClick={()=>setSelectedCardIds(new Set())} style={{background:"rgba(255,255,255,.15)",color:T.white,border:"1px solid rgba(255,255,255,.3)",borderRadius:8,padding:"6px 10px",cursor:"pointer",fontSize:12,fontWeight:700}}>✕ Clear</button>
              </>
            )}
            <button onClick={()=>exportPDF(room)} style={{background:T.orange,color:T.white,border:"none",borderRadius:12,padding:"9px 18px",fontWeight:700,fontSize:13,cursor:"pointer"}}>📥 PDF</button>
          </div>
        </div>
          ...
          <div style={{display:"flex",gap:14,alignItems:"flex-start",flexWrap:"wrap"}}>
            {COLUMNS.map(col=>(
              <BoardColumn key={col} col={col}
                ...
                onMove={moveEntry} onReorder={reorderEntry} onAddAction={addAction}
