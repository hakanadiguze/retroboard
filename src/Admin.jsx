        ...
        <div style={{background:`linear-gradient(135deg,${T.tealDark},${T.teal})`,padding:"20px 28px",display:"flex",alignItems:"center",gap:16}}>
          <div>
            ...
            <div style={{color:T.tealLight,fontSize:13,marginTop:2}}>
              {fmt(room.createdAt)} · Host: {room.hostName} · {parts.length} katılımcı
              {room.teamName&&<span style={{marginLeft:8,background:"rgba(255,255,255,.2)",borderRadius:6,padding:"1px 8px"}}>{room.teamName}</span>}
            </div>
          </div>
          <button onClick={onClose} style={{marginLeft:"auto",background:"rgba(255,255,255,.2)",border:"none",color:T.white,borderRadius:10,padding:"8px 14px",cursor:"pointer",fontWeight:700,fontSize:14}}>✕ Kapat</button>
        </div>

        <div style={{padding:28}}>
          {/* Score cards */}
          <h3 style={{margin:"0 0 14px",color:T.tealDark,fontSize:16,fontWeight:800}}>📊 Puan Ortalamaları</h3>
          ...
          {parts.length>0&&(
            <>
              <h3 style={{margin:"0 0 12px",color:T.tealDark,fontSize:16,fontWeight:800}}>👥 Katılımcı Puanları</h3>
                ...
                <table style={{width:"100%",borderCollapse:"collapse"}}>
                  <thead>
                    <tr style={{background:T.teal}}>
                      <th style={{padding:"9px 14px",textAlign:"left",color:T.white,fontSize:12,fontWeight:700,minWidth:160}}>Soru</th>
              ...
              return(
                <div key={col} style={{flex:"1 1 220px",borderRadius:14,overflow:"hidden",boxShadow:`0 2px 12px ${c}20`}}>
                  ...
                  <div style={{background:T.offWhite,padding:10,minHeight:60}}>
                    ...
                    {cards.length===0&&<div style={{textAlign:"center",color:T.gray300,fontSize:11,padding:"12px 0"}}>Kart yok</div>}
                  </div>
                  {actions.length>0&&(
                    <div style={{background:`${c}10`,padding:"8px 10px",borderTop:`1px solid ${c}25`}}>
                      <div style={{fontSize:10,fontWeight:800,color:c,marginBottom:4}}>⚡ AKSİYONLAR</div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}
      ...
      <div style={{width:"100%",maxWidth:560,background:T.white,borderRadius:20,boxShadow:"0 20px 60px rgba(0,0,0,.2)",overflow:"hidden"}}>
        <div style={{background:`linear-gradient(135deg,${T.tealDark},${T.teal})`,padding:"18px 24px",display:"flex",alignItems:"center"}}>
          <span style={{color:T.white,fontWeight:900,fontSize:18}}>👥 Takımlar</span>
        ...
        <div style={{padding:24}}>
          ...
          {teams.length>0&&(
            <div style={{marginBottom:20}}>
              <div style={{fontWeight:700,fontSize:13,color:T.gray500,marginBottom:8}}>Mevcut Takımlar</div>
              {teams.map((t,i)=>(
          ...
          <div style={{borderTop:teams.length>0?`1.5px solid ${T.gray100}`:"none",paddingTop:teams.length>0?20:0}}>
            <div style={{fontWeight:700,fontSize:13,color:T.gray500,marginBottom:10}}>Yeni Takım Ekle</div>
            <input value={name} onChange={e=>setName(e.target.value)} placeholder="Takım adı *" style={{...inp,marginBottom:8}}/>
            <input value={desc} onChange={e=>setDesc(e.target.value)} placeholder="Açıklama (opsiyonel)" style={{...inp,marginBottom:14}}/>
            <button onClick={handleSave} disabled={saving||!name.trim()}
              ...
              {saved?"✅ Kaydedildi!":saving?"Kaydediliyor…":"+ Takım Oluştur"}
            </button>
          </div>
        </div>
      </div>
...
export default function AdminPanel({ user }) {
  ...
  const filtered = rooms.filter(r=>{
    ...
    const matchSearch = !search || r.id.includes(search) || r.hostName?.toLowerCase().includes(search.toLowerCase()) || r.teamName?.toLowerCase().includes(search.toLowerCase());
    ...
    <div style={{minHeight:"100vh",background:"#E8F8F5",fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      ...
      <div style={{maxWidth:1200,margin:"0 auto",padding:"24px 20px"}}>
        ...
        <div style={{display:"flex",gap:14,marginBottom:24,flexWrap:"wrap"}}>
          {[
            {label:"Toplam Retro", value:stats.total, color:T.teal, icon:"🔄"},
            {label:"Tamamlanan",   value:stats.revealed, color:"#10B981", icon:"✅"},
            {label:"Devam Eden",   value:stats.total-stats.revealed, color:T.orange, icon:"⏳"},
            {label:"Toplam Katılım",value:stats.participants, color:"#8B5CF6", icon:"👥"},
          ].map(s=>(
        ...
        <div style={{background:T.white,borderRadius:14,padding:"14px 18px",marginBottom:16,display:"flex",gap:12,flexWrap:"wrap",alignItems:"center",boxShadow:`0 2px 10px ${T.teal}10`}}>
          <input value={search} onChange={e=>setSearch(e.target.value)}
            placeholder="🔍 Room ID, host veya takım ara…"
          ...
          <select value={filterTeam} onChange={e=>setFilterTeam(e.target.value)}
            ...
            <option value="none">Takım Yok</option>
            {teams.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}
          </select>
          <div style={{fontSize:13,color:T.gray500,whiteSpace:"nowrap"}}>{filtered.length} retro</div>
        </div>

        {/* Retro list */}
        {loading?(
          <div style={{textAlign:"center",padding:"60px 0",color:T.gray300,fontSize:16}}>Yükleniyor…</div>
        ):(
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {filtered.length===0&&(
              <div style={{textAlign:"center",padding:"60px 0",color:T.gray300,fontSize:15}}>Retro bulunamadı</div>
            )}
            {filtered.map(room=>{
              const parts=Object.values(room.participants||{}).filter(p=>p.submitted);
              ...
              return(
                <div key={room.id}
                  ...
                  style={{background:T.white,borderRadius:14,padding:"16px 20px",display:"flex",alignItems:"center",gap:16,cursor:"pointer",
                    boxShadow:`0 2px 10px ${T.teal}10`,border:`1.5px solid transparent`,
                    transition:"all .15s"}}
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                      ...
                      <span style={{background:room.revealed?"#D1FAE5":"#FEF3C7",color:room.revealed?"#065F46":"#92400E",borderRadius:6,padding:"1px 8px",fontSize:11,fontWeight:700}}>
                        {room.revealed?"Tamamlandı":"Devam Ediyor"}
                      </span>
                    </div>
                    <div style={{fontSize:12,color:T.gray500,marginTop:3}}>
                      Host: {room.hostName} · {fmt(room.createdAt)} · {parts.length} katılımcı
                    </div>
                  </div>
                  <div style={{fontSize:12,color:T.gray300,flexShrink:0}}>Detay →</div>
                </div>
              );
            })}
          </div>
